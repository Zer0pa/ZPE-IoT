...
- [x] Strict falsification rerun refreshed:
  - `zpe-iot/validation/results/dt_results_20260219T030940.json`
  - Summary: `PASS=21, FAIL=0, SKIPPED=0, BLOCKED=0`.
- [x] Baseline immutability contract remained enforced (`DT-16 PASS` with pinned tag path under `validation/results/baseline/`).
- [x] Provenance integrity remained green (`DT-17 PASS`) with DS-01..DS-08 `real_public` manifest entries.
- [x] Claim-tier compliance remained green (`DT-19 PASS`) with zero-tier `NOT_AVAILABLE` behavior intact.
- [x] Workstream ablation gate remained green (`DT-20 PASS`) with updated WI artifact:
  - `zpe-iot/validation/results/wi1_ablation_20260219T025552.json`.
