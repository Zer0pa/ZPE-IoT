#!/usr/bin/env python3
"""Run WI-1 ablation against baseline with full wire-path timings and strict-gate differential."""
...
THRESHOLD_CR_IMPROVEMENT = 0.12


@contextmanager
def _wi1_flag(enabled: bool):
    key = "ZPE_IOT_WI1_ENTROPY_STAGE"
    old = os.getenv(key)
    try:
        if enabled:
            os.environ[key] = "1"
...
def _wire_metrics(samples: np.ndarray, preset: str, repeats: int, warmup: int) -> tuple[dict, list[float], list[float]]:
    ...
    for run_idx in range(total_runs):
        ...
        for w in windows:
            ...
            err = float(compute_nrmse(w, restored))
            errs.append(err)
            if measure:
        ...
        if measure:
            ...
            run_raw.append(raw_bytes)
            run_packed.append(packed_bytes)
    ...
    metrics = {
        ...
        "raw_bytes_mean": int(np.mean(run_raw) if run_raw else 0),
        "packed_bytes_mean": int(np.mean(run_packed) if run_packed else 0),
    }
    return metrics, enc_times, dec_times


def _measure(ds_id: str, repeats: int, max_samples: int, warmup: int) -> tuple[dict, dict, dict, list[float], list[float], list[float], list[float]]:
    samples = load_dataset(ds_id)["samples"][: max(256, max_samples)]
    preset = ds_preset(ds_id)

    with _wi1_flag(False):
    ...
    aggregate = {
        "baseline_encode_p50_ms": float(np.percentile(base_enc, 50) if base_enc else 0.0),
        "baseline_encode_p99_ms": float(np.percentile(base_enc, 99) if base_enc else 0.0),
        "baseline_decode_p50_ms": float(np.percentile(base_dec, 50) if base_dec else 0.0),
        "baseline_decode_p99_ms": float(np.percentile(base_dec, 99) if base_dec else 0.0),
        "candidate_encode_p50_ms": float(np.percentile(cand_enc, 50) if cand_enc else 0.0),
        "candidate_encode_p99_ms": float(np.percentile(cand_enc, 99) if cand_enc else 0.0),
        "candidate_decode_p50_ms": float(np.percentile(cand_dec, 50) if cand_dec else 0.0),
        "candidate_decode_p99_ms": float(np.percentile(cand_dec, 99) if cand_dec else 0.0),
    }
    return baseline, candidate, aggregate, base_enc, base_dec, cand_enc, cand_dec
...
def _run_strict_dt(wi1_enabled: bool) -> dict:
    env = os.environ.copy()
    env["ZPE_IOT_WI1_ENTROPY_STAGE"] = "1" if wi1_enabled else "0"
...
def main() -> int:
    ...
    if not ds_ids:
        raise RuntimeError("No E1 datasets available for WI-1 ablation")
    ...
    for ds_id in ds_ids:
        b, c, latency, base_enc, base_dec, cand_enc, cand_dec = _measure(
            ds_id,
            repeats=max(5, args.repeats),
            max_samples=max(256, args.max_samples),
        ...
        rows.append(
            {
                ...
                "baseline": b,
                "wi1_candidate": c,
                "latency_wire_ms": latency,
                "cr_improvement": (c["cr"] - b["cr"]) / max(1e-9, b["cr"]),
                "nrmse_delta": c["nrmse"] - b["nrmse"],
            }
        )

    base_mean_cr = float(np.mean([r["baseline"]["cr"] for r in rows]))
    cand_mean_cr = float(np.mean([r["wi1_candidate"]["cr"] for r in rows]))
    base_mean_nrmse = float(np.mean([r["baseline"]["nrmse"] for r in rows]))
    cand_mean_nrmse = float(np.mean([r["wi1_candidate"]["nrmse"] for r in rows]))

    cr_gain = (cand_mean_cr - base_mean_cr) / max(1e-9, base_mean_cr)
    nrmse_delta = cand_mean_nrmse - base_mean_nrmse

    strict_baseline = _run_strict_dt(wi1_enabled=False)
    strict_candidate = _run_strict_dt(wi1_enabled=True)
    ...
    reason = (
        f"Retained: mean CR gain {cr_gain:.2%} >= {THRESHOLD_CR_IMPROVEMENT:.0%}, no NRMSE regression, and strict-gate differential clean"
        if retained
        else (
            f"Rejected: gain={cr_gain:.2%}, nrmse_delta={nrmse_delta:.3e}, "
            f"gate_regression_detected={gate_regression_detected}"
        )
    )
    ...
    payload = {
        ...
        "retained": retained,
        "reason": reason,
        "feature_flag": "ZPE_IOT_WI1_ENTROPY_STAGE",
    ...
    out = RESULTS_DIR / f"wi1_ablation_{payload['timestamp']}.json"
