...
VERSION_Y = 2
VERSION_LEN = 3
# Frame and delta control words reuse VERSION_X/Y on-wire and are only
# interpreted after the fixed header/x/y/len control sequence.
VERSION_FRAME = VERSION_X
VERSION_DELTA = VERSION_Y

MENTAL_TYPE_BIT = 0x0100
WORD_MASK = (1 << 20) - 1
DRAW_RLE_VERSION = 1
# Legacy flags kept for backward-compatible decode of historical artifacts.
LEGACY_FRAME_WORD_FLAG = 0x0800
LEGACY_RLE_RUN_FLAG = 0x0400
LEGACY_DELTA_WORD_FLAG = 0x0200
...
def _frame_payload(frame_index: int) -> int:
    return MENTAL_TYPE_BIT | (frame_index & 0xFF)


def _delta_payload(delta_ms: int) -> int:
    return MENTAL_TYPE_BIT | (delta_ms & 0xFF)


def _is_frame_word(mode: int, version: int, payload: int) -> bool:
    if mode != MODE_CONTROL or (payload & MENTAL_TYPE_BIT) == 0:
        return False
    if version == VERSION_FRAME:
        return (payload & (LEGACY_FRAME_WORD_FLAG | LEGACY_DELTA_WORD_FLAG)) == 0
    return version == VERSION_LEN and (payload & LEGACY_FRAME_WORD_FLAG) != 0
...
def _is_delta_word(mode: int, version: int, payload: int) -> bool:
    ...
    if version == VERSION_DELTA:
        return (payload & (LEGACY_FRAME_WORD_FLAG | LEGACY_DELTA_WORD_FLAG)) == 0
    return version == VERSION_LEN and (payload & LEGACY_DELTA_WORD_FLAG) != 0
...
def _pack_mental_strokes_raw(
    ...
    for stroke in strokes:
        ...
        for direction in draw_dirs:
            words.append(_build_word(MODE_DRAW, 0, _draw_payload(direction, profile)))
...
def unpack_mental_words(words: Iterable[int]) -> tuple[dict | None, List[MentalStroke]]:
    ...
    while i < len(data):
        ...
        for j in range(available):
            ...
            is_rle = d_version == DRAW_RLE_VERSION or (d_payload & LEGACY_RLE_RUN_FLAG) != 0
            if is_rle:
                encoding = "rle"
...
def pack_mental_strokes_rle(
    ...
    for stroke in strokes:
        ...
        for direction, run_length in runs:
            ...
            words.append(_build_word(MODE_DRAW, DRAW_RLE_VERSION, payload))
